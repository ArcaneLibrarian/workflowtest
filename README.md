...
This repo includes a reusable GitHub Actions workflow to generate summary statistics from the journal entry Excel export (`je_samples (1).xlsx`). The workflow writes outputs to an `outputs/` folder and uploads the folder as a workflow artifact so you can download the results.

## How it works

- The analysis script lives at `scripts/analyze_je.py`.
